...
import { 
  Search, Globe, Building2, Cpu, Filter, X, ChevronDown, 
  Download, Hash 
} from "lucide-react";

/**
 * Main Fiber AI Search Page
 * -------------------------------------------------------------
 * This component provides:
 * - Technology-based filtering (include, AND/OR/NOT)
 * - Firmographic filters (country, industry, min tech count)
 * - API-powered search (POST /api/search)
 * - Result table with CSV + JSON export
...
export default function Home() {
  ...
  const [techOperator, setTechOperator] = useState("OR"); // AND / OR / NOT
  ...
  /* ---------------------------------------------------------
     Fetch search results (POST request to backend)
     Triggered manually & on first load
  --------------------------------------------------------- */
  ...
  return (
    <div className="flex h-screen bg-slate-50 text-slate-900 font-sans">
      ...
      {/* -----------------------------------------------------
         SIDEBAR — Filters & Search Controls
      ----------------------------------------------------- */}
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <form onSubmit={addTech} className="relative">
              ...
              <button 
                type="submit" 
                className="absolute right-2 top-1.5 bg-slate-100 text-slate-600 px-2 py-0.5 rounded text-xs font-bold hover:bg-slate-200"
              >
                Add
              </button>
            </form>
            ...
            <div className="flex flex-wrap gap-1.5">
              {includeTech.map((t) => (
                <span 
                  key={t} 
                  className="bg-blue-50 text-blue-700 px-2 py-1 rounded-md text-xs font-semibold flex items-center gap-1.5 border border-blue-100"
                >
                  {t} 
                  <X size={12} className="cursor-pointer hover:text-blue-900" onClick={() => removeTech(t)} />
                </span>
              ))}
            </div>

            {/* Tech Operator Selector */}
            <select 
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-1.5">
                        {company.technologies?.slice(0, 4).map((t: any) => (
                          <span 
                            key={t.name} 
                            className="bg-white border border-slate-200 text-slate-700 px-2 py-0.5 rounded text-[10px] font-bold uppercase shadow-sm"
                          >
                            {t.name}
                          </span>
                        ))}
